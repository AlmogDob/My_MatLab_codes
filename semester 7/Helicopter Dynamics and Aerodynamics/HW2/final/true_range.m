...
% Estimates the true range
% Inputs - h [ft], W [lb], num_of_steps [-]
% Output - range [m]
...
Ws = [];
Ws = linspace(Total_Minimal_TO_Weight, W, num_of_steps); % [lb]

% Best_Specific_Range_AvgW = [];
% for i = 1:length(Ws)
%    [Best_Specific_Range_AvgW(i), ~] = Reduced_Specific_Range(h, Rotor_Speed, Ws(i));
% end
% range = trapz(Ws./2.2046, 1/C_sp_SI.*Best_Specific_Range_AvgW./(Ws.*9.81./2.2046)); % [m]
