...
for i = 1:length(AirSpeed)
    for j = 1:length(RPM)
        ...
        Q_Percent_Hover_RPM(i, j) = Hovering_Power_Oge(Average_Weight, Height(2)/1000, RPM(j)); % [%Q]
        Q_Percent_Hover_Height(i, j) = Hovering_Power_Oge(Average_Weight, Height(j)/1000, RPM(1)); % [%Q]
    end
    Limitation(i) = 85;
end
...
text(AirSpeed(35), 89, txt)
% exportgraphics(Figure1, 'Req_Power_Rotor_Speed.png','Resolution',900);
...
text(AirSpeed(35), 89, txt)
% exportgraphics(Figure2, 'Req_Power_Height.png','Resolution',900);
...
% 100% rotor speed
Ceiling_AvgW = Operational_Ceiling(Average_Weight, Rotor_Speed, 100) % [ft]
Ceiling_MaxW = Operational_Ceiling(Max_TO_Weight*2.2046, Rotor_Speed, 100) % [ft]
...
% exportgraphics(Figure3, 'Rsp.png','Resolution',900);
...
% exportgraphics(Figure4, 'V_For_Rsp.png','Resolution',900);
%%
% Rsp,V_Star(h)
Figure5 = figure;
hold on
...
ColorBar = colorbar;
ColorBar.Label.String = 'Height [ft]';
xlabel('Best speed for maximal reduced specific range [KCAS]')
ylabel('Maximal reduced specific range [-]')
legend('Average weight - 8900 [lb]', 'Maximal weight - 11200 [lb]', 'Location', 'southwest')
title('Maximal reduced specific range and best speed at 100% rotor speed', 'Almog Dobrescu - Ronnel Nawy')
% exportgraphics(Figure5, 'V_And_Rsp.png','Resolution',900);

%% Q5 - Real range

hs = linspace(0, Height_AvgW(end), 100); % [ft];
num_of_steps = 5;
range = [];
for i = 1:length(hs)
    range(i) = true_range(hs(i), Average_Weight, num_of_steps)*0.00054; % [Nm]
end

Figure6 = figure;
plot(hs,range, 'LineWidth', 2)
ylabel('True range [Nm]')
xlabel('Height [ft]')
title('True range for average weight', 'Almog Dobrescu - Ronnel Nawy')
% exportgraphics(Figure6, 'True_Range.png','Resolution',600);