...
state = [x0_vec;x0_vec;x0_vec]; % [x3;v3;x4;v4;x6;v6]
...
miss_distance = norm([x6_vec_out(end,1),x6_vec_out(end,2),x6_vec_out(end,3)]);
miss_velocity = norm([v6_vec_out(end,1),v6_vec_out(end,2),v6_vec_out(end,3)]);

fig1 = figure ("Name","3D Figure of The Orbit Trajectory",'Position',[100 300 900 500]);
colors = cool(3)*0.9;
plot3(x6_vec_out(:,1),x6_vec_out(:,2),x6_vec_out(:,3), "LineWidth", 2, "Color", colors(1,:))
hold all
plot3(x4_vec_out(:,1),x4_vec_out(:,2),x4_vec_out(:,3), "LineWidth", 2, "Color", colors(2,:))
plot3(x3_vec_out(:,1),x3_vec_out(:,2),x3_vec_out(:,3), "LineWidth", 2, "Color", colors(3,:))

plot3(x6_vec_out(1,1),x6_vec_out(1,2),x6_vec_out(1,3), "o", "LineWidth", 4, "Color", "k")
plot3(x6_vec_out(end,1),x6_vec_out(end,2),x6_vec_out(end,3),'pentagram', "LineWidth", 2, "Color", "r")
...
legend({'LQR', 'Pole Placement', 'Cross Product', 'init position', 'end position'},'FontSize',11 ,'Location','northwest')
...
hold all
% 6
plot(t_out, x6_vec_out(:,1),"-", "LineWidth", 1.5, "Color", lqr_colors(1,:))
plot(t_out, x6_vec_out(:,2),"-", "LineWidth", 1.5, "Color", lqr_colors(2,:))
plot(t_out, x6_vec_out(:,3),"-", "LineWidth", 1.5, "Color", lqr_colors(3,:))
plot(t_out, norm_x6,"-", "LineWidth", 1.5, "Color", lqr_colors(4,:))
% 4
...
subtitle("Almog Dobrescu 214254252")
legend({'LQR-x', 'LQR-y', 'LQR-z', 'LQR-norm', 'Pole Placement-x', 'Pole Placement-y', 'Pole Placement-z', 'Pole Placement-norm', 'Cross Product-x', 'Cross Product-y', 'Cross Product-z', 'Cross Product-norm'},'FontSize',11 ,'Location','northeast')
...
subtitle("Almog Dobrescu 214254252")
legend({'LQR-fx', 'LQR-fy', 'LQR-fz', 'LQR-f_norm', 'Pole Placement-fx', 'Pole Placement-fy', 'Pole Placement-fz', 'Pole Placement-f_norm', 'Cross Product-fx', 'Cross Product-fy', 'Cross Product-fz', 'Cross Product-f_norm', 'f_max'},'FontSize',11 ,'Location','northeast')
...
subtitle("Almog Dobrescu 214254252")
legend({'LQR-$\Delta v$', 'Pole Placement-$\Delta v$', 'Cross Product-$\Delta v$'},'FontSize',11 ,'Location','northeast', "Interpreter","latex")
